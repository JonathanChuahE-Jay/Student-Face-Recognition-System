...
const handleDisplayDailyReport = (database) => async (req, res) => {
    ...
    try {
        ...
        const currentDay = moment(date).tz('Asia/Kuala_Lumpur').format('dddd');
        ...
        const totalStudentsCurrentDayMap = await database('student_subjects')
            ...
            .leftJoin('sections', function () {
                this.on('sections.subject_id', '=', 'student_subjects.subject_id')
                    .andOn('sections.section_number', '=', 'student_subjects.subject_section');
            })
            .where('students.current_year', '=', database.raw('CAST(student_subjects.year AS INTEGER)'))
            .where('students.current_semester', '=', database.raw('CAST(student_subjects.semester AS INTEGER)'))
            .where('sections.day', '=', currentDay)
        ...
        const subjectsWithSectionsAndLecturers = subjects.map((subject) => {
            ...
            const sectionsWithLecturers = subjectSections.map((section) => {
                ...
                return {
                    ...
                    totalStudentsCurrentDay: totalStudentsCurrentDayMap[key] || 0
                };
            });
        ...
        if (result.length === 0 && subjectsAndSections && subjectsAndSections.length > 0) {
            result = [];
